...
def select_segmentation_from_valid(all_segmentation_dic):
    ...
    try:
        selected_segmentation_name_current_index =\
            valid_options_to_select.index(st.session_state.get('selected_segmentation_name',None))
    except:
        selected_segmentation_name_current_index = 0
    selected_segmentation_name = st.radio("Choose a segmentation to proceed ", options=valid_options_to_select, index=selected_segmentation_name_current_index, key='selected_segmentation_name' )
...
def save_button_clicked():
    st.session_state['fix_everything_and_show_image']= True
    st.session_state['disable_save_button'] = True
    st.session_state['disable_edit_button'] = False
    save_pickle_cache()

def edit_button_clicked():
    st.session_state['fix_everything_and_show_image']= False
    st.session_state['disable_save_button'] = False
    st.session_state['disable_edit_button'] = True
    save_pickle_cache()    
    load_pickle_file()
#-------- Streamlit App --------#
def main():
    ...
    fix_everything_and_show_image = st.session_state.get('fix_everything_and_show_image', False)
    fix_everything_and_show_image = st.session_state.get('disable_save_button', False)
    if fix_everything_and_show_image is False:
        with st.sidebar:
            st.button("Clear previous cache and start clean", on_click=reset_pickle_chace)
            ...
            if project_directory:
                ...
                if patient_folders:
                    try:
                        pt_directory_current_index = list(patient_folders.keys()).index(st.session_state['selected_patient_directory'])
                    except:
                        pt_directory_current_index=0
                    st.session_state['selected_patient_directory'] = st.selectbox("Select a patient folder", options=list(patient_folders.keys()),index= pt_directory_current_index)
                    st.session_state['selected_patient_directory'] = patient_folders[st.session_state['selected_patient_directory']]
                    if patient_folders:
                        ass_path = assessors_path(os.path.join(project_directory, st.session_state['selected_patient_directory']))                    
                        ...
                        if st.session_state["selected_segmentation_path"]:
                            ...
                            if selected_SEGdicom_data:
                                ...
                                if selected_segmentation_label:
                                    ...
                                    st.success("Segmentation data successfully loaded")           
                                    ...
                                    st.session_state['prepared_images'] = prepare_images(merged_data, segment_overlay_transparency=0.4, segment_overlay_color='#FFA500', figsize=(10, 10), label_for_overlay_image=st.session_state['segment_label'])
                                    
                                    st.button("Show the merged image and save it", on_click=save_button_clicked, disabled=st.session_state.get('disable_save_button', False),type='primary')
                                    
    if fix_everything_and_show_image is True:                       
        st.button("Edit and see another data", on_click=edit_button_clicked, disabled=st.session_state.get('disable_edit_button', True),type='primary')     
        show_merged_dicom_stack_fast(st.session_state['prepared_images'])
